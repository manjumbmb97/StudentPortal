		...
		if request.user.applicant:
			if request.method == 'POST':
				...
				if form.is_valid():
					...
					if form.cleaned_data['group_strength'] <= 10:
						slot = get_object_or_404(Slot, pk=form.cleaned_data['slot'].pk)
						if slot.is_filled == True:
							messages.error(request, "Selected slot has been filled")
							return redirect('portal:internship-form')
						else:
							slot.present_strength = slot.present_strength + form.cleaned_data['group_strength']
							if slot.present_strength == slot.max_strength:
								slot.is_filled = True
							slot.save()
						if slot.duration != form.cleaned_data['duration']:
							messages.error(request, "your duration and slot duration doesn't match. Please select a valid one")
							return redirect('portal:internship-form')
						applicant = get_object_or_404(Applicant, pk=request.user.applicant.id)
						applicant.is_filled = True
						applicant.internship_type = form.cleaned_data['internship_type']
						applicant.duration = form.cleaned_data['duration']
						applicant.slot = form.cleaned_data['slot']
						applicant.course = form.cleaned_data['course']
						applicant.department = form.cleaned_data['department']
						applicant.group_strength = form.cleaned_data['group_strength']
						applicant.save()
						for strength in range(applicant.group_strength):
							application = Application.objects.create(user=request.user)
							application.application_no =  str(datetime.date.today().year)[-2:]+applicant.department+str(format(application.id, "04"))
							application.internship_type = form.cleaned_data['internship_type']
							application.duration = form.cleaned_data['duration']
						...
						messages.error(request,"Group strength should be less than 10")
						return redirect('portal:internship-form')
			else:
				form = InternshipForm()
